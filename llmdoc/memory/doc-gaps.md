...
- **Stale count comment in `.github/tl_packages:31`.** The header comment says "22 entries" for the cls `\RequirePackage` block; the actual block has 27 lines. Not load-bearing (the whitelist filter doesn't see this file) but worth fixing on the next regen pass.
